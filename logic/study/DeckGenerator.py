...
class DeckGenerator:
    ...
    def generate(self) -> Deck:
        ...
        self.generate_flashcards(self.deck_position_dict['moves'])
        t2 = time()
    ...
    def generate_flashcards(self, moves: List[str], turn_depth: int, response_depth: int, player_color: Color = Color.WHITE) -> None:
        if turn_depth == 0:
            return
        color_to_move = Color.WHITE if len(moves) % 2 == 0 else Color.BLACK
        player_to_move = color_to_move.value == player_color
        ...
        if player_to_move:
            ...
            self.generate_flashcards(new_moves, turn_depth, response_depth, player_color)
        else: # Opponent to move
            top_moves = self.stockfish.get_top_moves(response_depth)
            centipawns = [x['Centipawn'] for x in top_moves]
            best_centipawn = min(centipawns) if player_color == Color.WHITE else max(centipawns) # i.e. Centipawn value of opponent's best move
            filter_good_moves = lambda x: x['Centipawn'] < best_centipawn + 100 if player_color == Color.WHITE else lambda x: x['Centipawn'] > best_centipawn - 100
