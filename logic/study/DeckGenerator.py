...
class DeckGenerator:
    ...
    def generate(self, stockfish: Stockfish) -> Deck:
        ...
        self.generate_flashcards(stockfish, self.deck_position_dict['moves'], self.turn_depth, self.response_depth, self.player_colour)
        self.database.commit()
        return self.deck

    def generate_flashcards(self, stockfish: Stockfish, moves: List[str], turn_depth: int, response_depth: int, player_colour: Colour = Colour.WHITE) -> None:
        if turn_depth == 0:
            return
        ...
        if player_to_move:
            ...
            self.generate_flashcards(stockfish, new_moves, turn_depth, response_depth, player_colour)
